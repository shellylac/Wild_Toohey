...
heatmapModuleServer <- function(id, filtered_data, taxa_level) {
  moduleServer(id, function(input, output, session) {
    ...
    period_filtered_data <- reactive({
      ...
      switch(
        input$heatmap_periods,
        'Spring' = {
          period_data |> filter(eventMonth %in% c(9, 10, 11))
        },
        'Summer' = {
          period_data |> filter(eventMonth %in% c(12, 1, 2))
        },
        'Autumn' = {
          period_data |> filter(eventMonth %in% c(3, 4, 5))
        },
        'Winter' = {
          period_data |> filter(eventMonth %in% c(6, 7, 8))
        }
      )
    }) |>
    ...
    output$heatmap_legend <- renderUI({
      ...
      tags$div(
        ...
        tags$div(
          style = "font-weight: bold; font-size: 0.8em; margin-bottom: 2px;",
        ...
        tags$div(
          ...
          lapply(1:length(heatmap_colors), function(i) {
            ...
            tags$div(
              ...
              tags$div(
                style = sprintf(
                  "width: 10px; height: 10px; background: %s; margin-right: 1px;",
    ...
    observeEvent(period_filtered_data(), {
      ...
      if (nrow(df) == 0 && !first_load()) {
        showNotification(
          HTML(
            "No data available for current selection.<br>Try selecting a different period."
          ),
          type = "warning",
          duration = 2,
          id = "heatmap_warning"
